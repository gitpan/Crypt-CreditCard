...
binary field of a database, e.g., VARCHAR BINARY.
...
Before and after a credit card number is encrypted and decrypted, a check is
made as to the validity of the credit card number using a mod-10 check. An
error will be raised if the credit card number is invalid.
...
Gets or sets the keylength, which is usually one of: 128, 192 or 256. This
...
of a number, not a number (see CAVEATS). This method is optional.
...
(see CAVEATS). This method is optional.
...
Gets or sets the initial vector (see Crypt::Rijndael for details). This is
optional.
...
encrypt() is called against it.

By default, strength() is set to use /dev/random, however if you don't desire
blocking for the purpose of more random data, consider setting strength() to
use /dev/urandom. The following excerpt from Crypt::Random states, "The
/dev/random driver maintains an estimate of true randomness in the pool and
...
non-deterministic events to refresh the pool. When the routine is blocked,
Crypt::Random's read() will be blocked till desired amount of random bytes have
been read off of the device. The /dev/random kernel module also provides
another interface, /dev/urandom, that does not wait for the entropy-pool to
recharge and returns as many bytes as requested. For applications that must not
block (for a potentially long time) should use /dev/urandom. /dev/random should
be reserved for instances where very high quality randomness is desired. If
there's a hardware random number generator available, for instance the Intel
i8x0 random number generator, please use it instead of /dev/random! It'll be
high quality, a lot faster and it won't block! Usually your OS will provide
access to the RNG as a device, eg (/dev/intel_rng)."

IN ORDER TO BE GUARANTEED NOMINAL OPERATION, YOU MUST check for errstr() on the
following methods: mode(), strength(), keylength(), datasize(), cvv2(),
month(), year(), number(), encrypt(), decrypt().

=head1 BUGS

  Please report any bugs to http://rt.cpan.org/. Patches are most welcome.
